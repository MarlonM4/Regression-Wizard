...
"""
    ...
    def show_selected_stats(self):
        ...
        # Primero obtener el reporte base
        stats_report = result["stats_report"]
        
        # Luego añadir bootstrapping si existe
        if result.get("bootstrap_results"):
            ...
            stats_report += "="*60 + "\n"
            stats_report += f"Número de muestras bootstrap exitosas: {n_samples}\n\n"
            
            for name in result["param_names"]:
                if name in conf_intervals:
                    stats_report += (
                        f"{name}: {result['params'][result['param_names'].index(name)]:.6f}\n"
                        f"IC 95% bootstrap: [{conf_intervals[name][0]:.6f}, {conf_intervals[name][1]:.6f}]\n"
                        f"Error estándar bootstrap: {np.std(bootstrap_params[name]):.6f}\n\n"
                    )
                if n_samples == 0:
                    stats_report += "\nNo se obtuvieron muestras bootstrap válidas\n"
                    continue
        ...
        text_widget.insert(tk.END, stats_report)  # Usar stats_report actualizado
    ...
    def show_selected_plot(self):
        ...
        # Mostrar gráfico normal solo si el bootstrapping NO está activado
        if not self.bootstrap_var.get():
            self.show_normal_plot(result)
        else:
            # Si el bootstrapping está activado, mostrar los gráficos de distribución
            if result.get("bootstrap_results"):
                self.show_bootstrap_plots(result)   
        ...
        if result.get("bootstrap_results"):
            ...
            if n_samples > 0:
                # Calcular predicciones para cada conjunto de parámetros bootstrap
                ...
                # Dibujar bandas de confianza
                ax.fill_between(x_vals, lower, upper, color='red', alpha=0.2, 
                              label='Intervalo 95% de confianza')
    ...
    def show_normal_plot(self, result):
        """Muestra el gráfico normal de ajuste del modelo."""
        plot_window = tk.Toplevel(self.root)
        plot_window.title(f"Gráfico: {result['name']}")
        ...
        # Mover el código de bootstrapping aquí para evitar duplicación
        if result.get("bootstrap_results"):
            bootstrap_params = result["bootstrap_results"]["bootstrap_params"]
            n_samples = len(next(iter(bootstrap_params.values())))
