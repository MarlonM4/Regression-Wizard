...
class ModelApp:
    ...
    def setup_window(self):
        self.root.title("Regression Wizard - v0.9 fixing :3")
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Exportar gráficos", command=self.export_plots_to_pdf)
        ...
        help_menu.add_command(label="Manual de usuario", command=self.show_user_manual)
    ...
    def show_user_manual(self):
        """Muestra un mensaje indicando que el manual está en desarrollo"""
        messagebox.showinfo("Manual de usuario", "Coming soon (:P)")
    ...
    def create_model_selection(self, parent):
        ...
        self.all_models_btn = ttk.Button(  # Cambiar a variable de instancia
            button_frame, 
            text="PROCESAR TODOS LOS MODELOS", 
            style='Process.TButton',
            command=self.ask_min_r2
        )
        self.all_models_btn.pack(side="right", padx=5)
        ...
        try:
            ...
            tooltip.bind_widget(self.all_models_btn, 
        ...
        self.update_all_models_button()
            
        self.update_model_list()
    
    def update_all_models_button(self):
        """Actualiza la apariencia del botón según el estado de bootstrap"""
        if self.bootstrap_var.get():
            # Bootstrap activado - botón rojo
            self.all_models_btn.configure(style='Warning.TButton')
        else:
            # Bootstrap desactivado - estilo normal
            self.all_models_btn.configure(style='Process.TButton')
    ...
    def create_results_display(self, parent):
        ...
        ttk.Button(action_frame,
        text="Exportar Gráficos (PDF)",
        command=self.export_plots_to_pdf).pack(side="left", padx=5)
    ...
    def sort_treeview(self, column):
        ...
        if column == "Categoría":
            ...
            # Número más cercano a 1 primero (descendente)
            items.sort(key=lambda x: float(x[0]) if x[0].replace('.', '').isdigit() else 0, reverse=True)
        
        elif column == "Evaluación":
            # Orden personalizado: Excelente > Bueno > Moderado > Pobre
            eval_order = {"Excelente": 4, "Bueno": 3, "Moderado": 2, "Pobre": 1}
            items.sort(key=lambda x: eval_order.get(x[0], 0), reverse=True)
        
        elif column == "Parámetros":
            # Número más bajo de parámetros primero
            items.sort(key=lambda x: len(x[0].split(',')) if x[0] else 0)
        
        else:
            # Para "Modelo" u otras columnas: orden alfabético
            items.sort(key=lambda x: x[0].lower())
    ...
    def perform_bootstrap(self, func, x, y, params, param_names, n_iterations=200):
        ...
        try:
            # Asegurar que params sea una lista
            ...
            for _ in range(n_iterations):
                ...
                try:
                    ...
                    # Asegurar que popt sea una lista
                    if isinstance(popt, (set, np.ndarray)):
                        popt = list(popt)
                        
                    # Almacenar parámetros
                    for name, val in zip(param_names, popt):
                        bootstrap_params[name].append(float(val))  # Convertir a float explícitamente
            ...
            for name in param_names:
                if bootstrap_params[name]:
                    ...
                    numeric_values = [float(val) for val in bootstrap_params[name] if not isinstance(val, set)]
                    if numeric_values:
                        lower = np.percentile(numeric_values, 2.5)
                        upper = np.percentile(numeric_values, 97.5)
    ...
    def process_selected_model(self, suppress_alerts=False):
        ...
        try:
            ...
            # BOOTSTRAPPING - SIEMPRE EJECUTAR SI ESTÁ ACTIVADO
            # NO desactivar el bootstrap aunque se hayan procesado otros modelos antes
            bootstrap_results = None
            if self.bootstrap_var.get():  # Solo depende del checkbox actual
                try:
                    ...
                    # Asegurar que los parámetros bootstrap sean listas
            ...
            # Asegurar que los parámetros sean una lista, no un set
            if isinstance(params, (set, np.ndarray)):
                params = list(params)
            ...
            r2 = 1 - (np.sum((y - y_pred)**2) / np.sum((y - np.mean(y))**2)) if ss_tot != 0 else 0
    ...
    """
            ...
            self.results.append({
                "name": model_name,  # Guardar el nombre ORIGINAL sin asterisco
                "category": category,
                "params": params_list,
                "pcov": pcov,
                "r2": r2,
                ...
                "bootstrap_results": bootstrap_results  # Siempre guardar aunque sea None
            })
            
            # Mostrar en tabla
            display_name = model_name
            if self.bootstrap_var.get() and bootstrap_results is not None:
                display_name = model_name + " *"  # Añadir asterisco solo si hay bootstrap
            
            eval_text, eval_color = self.evaluate_r2(r2)
            param_text = ", ".join([f"{name}={val:.4f}" for name, val in zip(param_names, params)])
            
            item = self.tree.insert("", "end", values=(display_name, category, f"{r2:.4f}", eval_text, param_text))
    ...
    def get_model_equation(self, model_name, params, param_names):
        ...
        def format_param(val):
            # Convertir sets a valores numéricos
            if isinstance(val, set):
                if len(val) == 1:
                    return f"{next(iter(val)):.4f}"
                else:
        ...
        # Función auxiliar para obtener parámetros formateados de manera segura
        def get_param(index):
            if index < len(params):
                return format_param(params[index])
            return "0.0"
        
        try:
            # Modelos específicos que necesitan manejo especial
            if model_name == "Weibull 4 parámetros (dist)":
                a = get_param(0)
                b = get_param(1)
                c = get_param(2)
                x0 = get_param(3)
                
                # Calcular términos numéricamente seguros
                try:
                    c_val = float(c)
                    term_val = (c_val - 1) / c_val if c_val != 0 else 0
                    term = format_param(term_val)
                    term_power = format_param((1 - c_val) / c_val) if c_val != 0 else "0.0"
                    term_exp = format_param(1/c_val) if c_val != 0 else "0.0"
                except:
                    term = "0.0"
                    term_power = "0.0"
                    term_exp = "0.0"
                    
                return f"y = {a} * {term}^{term_power} * |(x-{x0})/{b} + {term}^{term_exp}|^{c_val-1} * exp(-|(x-{x0})/{b} + {term}^{term_exp}|^{c} + {term})"
                ...
                return f"y = {y0} + {a} * {term}^{term_power} * |(x-{x0})/{b} + {term}^{term_exp}|^{c_val-1} * exp(-|(x-{x0})/{b} + {term}^{term_exp}|^{c} + {term})"
            
            # Sigmoidales
            elif model_name == "Sigmoidal 3 parámetros":
                return f"y = {get_param(0)} / (1 + exp(-(x - {get_param(2)})/{get_param(1)}))"
            
            elif model_name == "Sigmoidal 4 parámetros":
                return f"y = {get_param(3)} + {get_param(0)} / (1 + exp(-(x - {get_param(2)})/{get_param(1)}))"
            
            elif model_name == "Sigmoidal 5 parámetros":
                return f"y = {get_param(4)} + ({get_param(0)} / (1 + exp(-(x - {get_param(3)})/{get_param(1)})))^{get_param(2)}"
            
            elif model_name == "Logística 3 parámetros":
                return f"y = {get_param(0)} / (1 + |x/{get_param(2)}|^{get_param(1)})"
            
            elif model_name == "Logística 4 parámetros":
                return f"y = {get_param(3)} + {get_param(0)} / (1 + |x/{get_param(2)}|^{get_param(1)})"
            
            elif model_name == "Weibull 4 parámetros":
                return f"y = {get_param(0)} * (1 - exp(-((x - {get_param(3)} + {get_param(1)}*(ln(2)^(1/{get_param(2)}))/{get_param(1)})^{get_param(2)}))"
            
            elif model_name == "Weibull 5 parámetros":
                return f"y = {get_param(4)} + {get_param(0)} * (1 - exp(-((x - {get_param(3)} + {get_param(1)}*(ln(2)^(1/{get_param(2)}))/{get_param(1)})^{get_param(2)}))"
            
            elif model_name == "Gompertz 3 parámetros":
                return f"y = {get_param(0)} * exp(-exp(-(x - {get_param(2)})/{get_param(1)}))"
            
            elif model_name == "Gompertz 4 parámetros":
                return f"y = {get_param(3)} + {get_param(0)} * exp(-exp(-(x - {get_param(2)})/{get_param(1)}))"
            
            elif model_name == "Hill 3 parámetros":
                return f"y = ({get_param(0)} * x^{get_param(1)}) / ({get_param(2)}^{get_param(1)} + x^{get_param(1)})"
            
            elif model_name == "Hill 4 parámetros":
                return f"y = {get_param(3)} + ({get_param(0)} * x^{get_param(1)}) / ({get_param(2)}^{get_param(1)} + x^{get_param(1)})"
            
            elif model_name == "Chapman 3 parámetros":
                return f"y = {get_param(0)} * (1 - exp(-{get_param(1)} * x))^{get_param(2)}"
            
            elif model_name == "Chapman 4 parámetros":
                return f"y = {get_param(3)} + {get_param(0)} * (1 - exp(-{get_param(1)} * x))^{get_param(2)}"
            
            # Polinomiales
            elif model_name == "Lineal":
                return f"y = {get_param(1)} + {get_param(0)} * x"
            
            elif model_name == "Cuadrático":
                return f"y = {get_param(0)} + {get_param(1)} * x + {get_param(2)} * x²"
            
            elif model_name == "Cúbico":
                return f"y = {get_param(0)} + {get_param(1)} * x + {get_param(2)} * x² + {get_param(3)} * x³"
            
            # Inversos
            elif model_name == "Inverso 1º orden":
                return f"y = {get_param(0)} + {get_param(1)} / x"
            ...
            # Distribuciones
                ...
                return f"y = {get_param(0)} * exp(-0.5 * ((x - {get_param(2)})/{get_param(1)})²)"
            
            elif model_name == "Gaussiana 4 parámetros":
                return f"y = {get_param(3)} + {get_param(0)} * exp(-0.5 * ((x - {get_param(2)})/{get_param(1)})²)"
            
            elif model_name == "Pseudo-Voigt 4 parámetros":
                return f"y = {get_param(0)} * ({get_param(2)} * (1/(1+((x-{get_param(3)})/{get_param(1)})²)) + (1-{get_param(2)}) * exp(-0.5*((x-{get_param(3)})/{get_param(1)})²))"
            
            elif model_name == "Pseudo-Voigt 5 parámetros":
                return f"y = {get_param(4)} + {get_param(0)} * ({get_param(2)} * (1/(1+((x-{get_param(3)})/{get_param(1)})²)) + (1-{get_param(2)}) * exp(-0.5*((x-{get_param(3)})/{get_param(1)})²))"
            
            elif model_name == "Gaussiana modificada 4 parámetros":
                return f"y = {get_param(0)} * exp(-0.5 * |(x-{get_param(3)})/{get_param(1)}|^{get_param(2)})"
            
            elif model_name == "Gaussiana modificada 5 parámetros":
                return f"y = {get_param(4)} + {get_param(0)} * exp(-0.5 * |(x-{get_param(3)})/{get_param(1)}|^{get_param(2)})"
                ...
                return f"y = {get_param(0)} * exp(-0.5 * (ln(x/{get_param(2)})/{get_param(1)})²) / x"
            
            elif model_name == "Log Normal 4 parámetros":
                return f"y = {get_param(3)} + {get_param(0)} * exp(-0.5 * (ln(x/{get_param(2)})/{get_param(1)})²) / x"
            
            # Decaimiento Exponencial
            elif model_name == "Exponencial simple 2p":
                return f"y = {get_param(0)} * exp(-{get_param(1)} * x)"
            
            elif model_name == "Exponencial con offset 3p":
                return f"y = {get_param(0)} + {get_param(1)} * exp(-{get_param(2)} * x)"
            
            elif model_name == "Doble exponencial 4p":
                return f"y = {get_param(0)} * exp(-{get_param(1)} * x) + {get_param(2)} * exp(-{get_param(3)} * x)"
            
            elif model_name == "Doble exponencial con offset 5p":
                return f"y = {get_param(0)} + {get_param(1)} * exp(-{get_param(2)} * x) + {get_param(3)} * exp(-{get_param(4)} * x)"
            
            elif model_name == "Triple exponencial 6p":
                return f"y = {get_param(0)} * exp(-{get_param(1)} * x) + {get_param(2)} * exp(-{get_param(3)} * x) + {get_param(4)} * exp(-{get_param(5)} * x)"
            
            elif model_name == "Triple exponencial con offset 7p":
                return f"y = {get_param(0)} + {get_param(1)} * exp(-{get_param(2)} * x) + {get_param(3)} * exp(-{get_param(4)} * x) + {get_param(5)} * exp(-{get_param(6)} * x)"
            
            elif model_name == "Combinación lineal-exponencial":
                return f"y = {get_param(0)} + {get_param(1)} * exp(-{get_param(2)} * x) + {get_param(3)} * x"
            
            elif model_name == "Exponencial modificado 3p":
                return f"y = {get_param(0)} * exp({get_param(1)} / (x + {get_param(2)}))"
            
            # Crecimiento Exponencial Limitado
            elif model_name == "Crec. Exp. L. Simple 2p":
                return f"y = {get_param(0)} * (1 - exp(-{get_param(1)} * x))"
            
            elif model_name == "Crec. Exp. L. Simple 3p":
                return f"y = {get_param(0)} + {get_param(1)} * (1 - exp(-{get_param(2)} * x))"
            
            elif model_name == "Crec. Exp. L. Doble 4p":
                return f"y = {get_param(0)} * (1 - exp(-{get_param(1)} * x)) + {get_param(2)} * (1 - exp(-{get_param(3)} * x))"
            
            elif model_name == "Crec. Exp. L. Doble 5p":
                return f"y = {get_param(0)} + {get_param(1)} * (1 - exp(-{get_param(2)} * x)) + {get_param(3)} * (1 - exp(-{get_param(4)} * x))"
            
            elif model_name == "Crec. Exp. L. Simple Alt 2p":
                return f"y = {get_param(0)} * (1 - {get_param(1)}^x)"
            
            elif model_name == "Crec. Exp. L. Simple Alt 3p":
                return f"y = {get_param(0)} + {get_param(1)} * (1 - {get_param(2)}^x)"
            
            # Modelos de Potencia
            elif model_name == "Potencia 2 parámetros":
                return f"y = {get_param(0)} * x^{get_param(1)}"
            
            elif model_name == "Potencia 2p Modificado I":
                return f"y = {get_param(0)} * (1 - x^{-get_param(1)})"
            
            elif model_name == "Potencia 2p Modificado II":
                return f"y = {get_param(0)} * (1 + x)^{get_param(1)}"
            
            elif model_name == "Potencia 3 parámetros":
                return f"y = {get_param(0)} + {get_param(1)} * x^{get_param(2)}"
            
            elif model_name == "Potencia Simétrica 3p":
                return f"y = {get_param(0)} * |x - {get_param(1)}|^{get_param(2)}"
            
            elif model_name == "Potencia Simétrica 4p":
                return f"y = {get_param(3)} + {get_param(0)} * |x - {get_param(1)}|^{get_param(2)}"
            
            elif model_name == "Función Pareto":
                return f"y = 1 - 1/x^{get_param(0)}"
            
            elif model_name == "Función Pareto Modificada":
                return f"y = 1 - 1/(1 + {get_param(0)} * x)^{get_param(1)}"
            
            # Ciencia-Bio
            elif model_name == "Beverton-Holt A (BH-A)":
                return f"y = {get_param(0)} / (1 + (({get_param(0)}-1)/{get_param(1)}) * x)"
            
            elif model_name == "Beverton-Holt B (BH-B)":
                return f"y = {get_param(0)} * x / (1 + (({get_param(0)}-1)/{get_param(1)}) * x)"
            
            elif model_name == "von Bertalanffy Growth (VBG)":
                return f"y = {get_param(0)} * (1 - exp(-{get_param(1)} * (x - {get_param(2)})))"
            
            elif model_name == "Michaelis-Menten básico (MM)":
                return f"y = {get_param(0)} * x / ({get_param(1)} + x)"
            
            elif model_name == "Michaelis-Menten doble (MMD)":
                return f"y = {get_param(0)} * x / ({get_param(1)} + x) + {get_param(2)} * x / ({get_param(3)} + x)"
            
            # Dose-Response models
            elif model_name == "Dose-Response A (DR-A)":
                return f"y = {get_param(1)} + ({get_param(0)} - {get_param(1)}) / (1 + 10^(x - {get_param(2)}))"
            
            elif model_name == "Dose-Response B (DR-B)":
                return f"y = {get_param(1)} + ({get_param(0)} - {get_param(1)}) / (1 + 10^({get_param(2)} - x))"
            
            elif model_name == "Dose-Response C (DR-C)":
                return f"y = {get_param(1)} + ({get_param(0)} - {get_param(1)}) / (1 + 10^({get_param(3)} * (x - {get_param(2)})))"
            
            elif model_name == "Dose-Response D (DR-D)":
                return f"y = {get_param(1)} + ({get_param(0)} - {get_param(1)}) / (1 + 10^({get_param(3)} * ({get_param(2)} - x)))"
            
            elif model_name == "Dose-Response E (DR-E)":
                return f"y = {get_param(1)} + ({get_param(0)} - {get_param(1)}) / (1 + (x/{get_param(2)})^{get_param(3)})"
            ...
            # Plant Disease models
                ...
                return f"y = exp(ln({get_param(0)}) * exp(-{get_param(1)} * x))"
            
            elif model_name == "Enfermedad vegetal Logística (PV-Log)":
                return f"y = 1 / (1 + (1 - {get_param(0)})/({get_param(0)} * exp(-{get_param(1)} * x)))"
            
            elif model_name == "Enfermedad vegetal Monomolecular (PV-Mono)":
                return f"y = 1 - ((1 - {get_param(0)}) * exp(-{get_param(1)} * x))"
            
            elif model_name == "Enfermedad vegetal Weibull (PV-Weib)":
                return f"y = 1 - exp(-1.0 * ((x - {get_param(0)})/{get_param(1)})^{get_param(2)})"
            
            # Affinity models
            elif model_name == "High-Low Affinity (HLA)":
                return f"y = {get_param(0)} * {get_param(1)} * x / (1 + {get_param(1)} * x)"
            
            elif model_name == "High-Low Affinity doble (HLAD)":
                return f"y = {get_param(0)} * {get_param(1)} * x / (1 + {get_param(1)} * x) + {get_param(2)} * {get_param(3)} * x / (1 + {get_param(3)} * x)"
            
            # Standard Logistic
            elif model_name == "Logística 3P estándar (Log3P)":
                return f"y = {get_param(2)} + ({get_param(0)} - {get_param(2)}) / (1 + x/{get_param(1)})"
            
            elif model_name == "Logística 4P estándar (Log4P)":
                return f"y = {get_param(3)} + ({get_param(0)} - {get_param(3)}) / (1 + (x/{get_param(2)})^{get_param(1)})"
            
            elif model_name == "Logística 5P estándar (Log5P)":
                return f"y = {get_param(3)} + ({get_param(0)} - {get_param(3)}) / (1 + (x/{get_param(2)})^{get_param(1)})^{get_param(4)}"
            
            # Otros modelos bio
            elif model_name == "Transporte de membrana (MT)":
                return f"y = {get_param(0)} * (x - {get_param(1)}) / (x² + {get_param(2)} * x + {get_param(3)})"
            
            elif model_name == "Aphid Population Growth (APHID)":
                return f"y = {get_param(0)} * exp({get_param(1)} * x) * (1 + {get_param(2)} * exp({get_param(1)} * x))^(-2)"
            
            elif model_name == "Negative Exponential generalizada (NEG)":
                return f"y = {get_param(0)} * (1.0 - exp(-{get_param(1)} * x))^{get_param(2)}"
            
            # Modelos NIST
            elif model_name == "Hyperbolic Tangent 4p (HT)":
                return f"y = ({get_param(0)} + {get_param(1)})/2 + ({get_param(1)} - {get_param(0)})/2 * tanh((x - {get_param(2)})/{get_param(3)})"
            
            elif model_name == "Hyperbolic Tangent 5p (HT)":
                return f"y = {get_param(4)} + ({get_param(0)} + {get_param(1)})/2 + ({get_param(1)} - {get_param(0)})/2 * tanh((x - {get_param(2)})/{get_param(3)})"
            
            elif model_name == "Asymmetric Hyperbolic Tangent 5p (AHT)":
                return f"y = ({get_param(0)} + {get_param(1)})/2 + ({get_param(1)} - {get_param(0)})/2 * tanh((x - {get_param(2)})/({get_param(3)} + {get_param(4)}*x))"
            
            elif model_name == "Asymmetric Hyperbolic Tangent 6p (AHT)":
                return f"y = {get_param(5)} + ({get_param(0)} + {get_param(1)})/2 + ({get_param(1)} - {get_param(0)})/2 * tanh((x - {get_param(2)})/({get_param(3)} + {get_param(4)}*x))"
            
            elif model_name == "Burr Model 5p (BUR)":
                return f"y = {get_param(0)} + ({get_param(1)} - {get_param(0)}) * [1 + exp(-{get_param(2)}*(x - {get_param(3)}))]^(-{get_param(4)})"
            
            elif model_name == "Burr Model 6p (BUR)":
                return f"y = {get_param(5)} + {get_param(0)} + ({get_param(1)} - {get_param(0)}) * [1 + exp(-{get_param(2)}*(x - {get_param(3)}))]^(-{get_param(4)})"
            
            elif model_name == "Arctangent Model 4p (ACT)":
                return f"y = ({get_param(0)} + {get_param(1)})/2 + ({get_param(1)} - {get_param(0)})/2 * arctan(π/(2*{get_param(3)}) * (x - {get_param(2)}))"
            
            elif model_name == "Arctangent Model 5p (ACT)":
                return f"y = {get_param(4)} + ({get_param(0)} + {get_param(1)})/2 + ({get_param(1)} - {get_param(0)})/2 * arctan(π/(2*{get_param(3)}) * (x - {get_param(2)}))"
            
            elif model_name == "Asymmetric Kohout Model 5p (KHT)":
                return f"Para x ≤ {get_param(3)}: y = {get_param(0)} + ({get_param(1)} - {get_param(0)})/(1 + {get_param(4)}) * exp((1 + {get_param(4)})/(2*{get_param(2)}) * (x - {get_param(3)}))\nPara x > {get_param(3)}: y = {get_param(0)} - {get_param(4)}*({get_param(1)} - {get_param(0)})/(1 + {get_param(4)}) * exp(-(1 + {get_param(4)})/(2*{get_param(2)}) * (x - {get_param(3)}))"
            
            elif model_name == "Asymmetric Kohout Model 6p (KHT)":
                return f"Para x ≤ {get_param(3)}: y = {get_param(5)} + {get_param(0)} + ({get_param(1)} - {get_param(0)})/(1 + {get_param(4)}) * exp((1 + {get_param(4)})/(2*{get_param(2)}) * (x - {get_param(3)}))\nPara x > {get_param(3)}: y = {get_param(5)} + {get_param(0)} - {get_param(4)}*({get_param(1)} - {get_param(0)})/(1 + {get_param(4)}) * exp(-(1 + {get_param(4)})/(2*{get_param(2)}) * (x - {get_param(3)}))"
            
            elif model_name == "Monotonic Four Parameter (MFP)":
                return f"y = {get_param(0)} + {get_param(2)} * (x - {get_param(1)}) * |x - {get_param(1)}|^({get_param(3)} - 1)"
            
            elif model_name == "Quadratic Four Parameter (QFP)":
                alpha = float(get_param(3))
                if abs(alpha) < 1e-10:
                    return f"y = {get_param(0)} + {get_param(1)} * ln(x) + {get_param(2)} * (ln(x))^2"
                else:
                    return f"y = {get_param(0)} + {get_param(1)} * (x^{alpha} - 1)/{alpha} + {get_param(2)} * [(x^{alpha} - 1)/{alpha}]^2"
            
            elif model_name == "SVD 2D 6 parámetros":
                return f"z = {get_param(0)} * {get_param(1)} * {get_param(3)} * x + {get_param(0)} * {get_param(2)} * {get_param(4)} * y"
            
            elif model_name == "SVD 2D 9 parámetros":
                return f"z = {get_param(0)} * {get_param(2)} * {get_param(6)} * x + {get_param(0)} * {get_param(3)} * {get_param(7)} * y + {get_param(1)} * {get_param(4)} * {get_param(6)} * x + {get_param(1)} * {get_param(5)} * {get_param(7)} * y"
            ...
            # Ingeniería 2D
                ...
                return f"y = {get_param(0)}*exp(-{get_param(1)}*x) + {get_param(2)}*exp(-((x-{get_param(3)})/{get_param(4)})²) + {get_param(5)}*exp(-((x-{get_param(6)})/{get_param(7)})²) + {get_param(8)}*exp(-((x-{get_param(9)})/{get_param(10)})²)"
            
            elif model_name == "Steinhart-Hart Extendido 2D":
                return f"y = {get_param(0)} + {get_param(1)}*ln(R) + {get_param(2)}*(ln(R))² + {get_param(3)}*(ln(R))³"
            
            elif model_name == "Motor Eléctrico Graeme Paterson 2D":
                return f"y = {get_param(0)}*exp(-{get_param(1)}*t)*cos({get_param(2)}*t + {get_param(3)}) + {get_param(4)}*exp(-{get_param(5)}*t)"
            
            elif model_name == "Klimpel Cinética Flotación A 2D":
                return f"y = {get_param(0)} * [1 - (1 - exp(-{get_param(1)}*x)) / ({get_param(1)}*x)]"
            
            elif model_name == "Maxwell-Wiechert 1 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)})"
            
            elif model_name == "Maxwell-Wiechert 2 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}*exp(-X/{get_param(3)})"
            
            elif model_name == "Maxwell-Wiechert 3 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}*exp(-X/{get_param(3)}) + {get_param(4)}*exp(-X/{get_param(5)})"
            
            elif model_name == "Maxwell-Wiechert 4 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}*exp(-X/{get_param(3)}) + {get_param(4)}*exp(-X/{get_param(5)}) + {get_param(6)}*exp(-X/{get_param(7)})"
            
            elif model_name == "Producción de Pozo Arps Modificado 2D":
                return f"y = ({get_param(0)}/((1.0-{get_param(1)})*{get_param(2)})) * (1.0-((1.0+{get_param(1)}*{get_param(2)}*x)^(1.0-1.0/{get_param(1)})))"
            
            elif model_name == "Ramberg-Osgood 2D":
                return f"y = (Stress/{get_param(0)}) + (Stress/{get_param(1)})^(1.0/{get_param(2)})"
            
            elif model_name == "Steinhart-Hart Extendido Recíproco 2D":
                return f"y = 1.0 / ({get_param(0)} + {get_param(1)}*ln(R) + {get_param(2)}*(ln(R))² + {get_param(3)}*(ln(R))³)"
            
            elif model_name == "Steinhart-Hart Recíproco 2D":
                return f"y = 1.0 / ({get_param(0)} + {get_param(1)}*ln(R) + {get_param(2)}*(ln(R))³)"
            
            elif model_name == "Sellmeier Óptico 2D":
                return f"y = 1 + ({get_param(0)}*x²)/(x²-{get_param(1)}) + ({get_param(2)}*x²)/(x²-{get_param(3)}) + ({get_param(4)}*x²)/(x²-{get_param(5)})"
            
            elif model_name == "Sellmeier Óptico Raíz Cuadrada 2D":
                return f"y = √[1 + ({get_param(0)}*x²)/(x²-{get_param(1)}) + ({get_param(2)}*x²)/(x²-{get_param(3)}) + ({get_param(4)}*x²)/(x²-{get_param(5)})]"
            
            elif model_name == "Steinhart-Hart 2D":
                return f"y = {get_param(0)} + {get_param(1)}*ln(R) + {get_param(2)}*(ln(R))³"
            ...
            # Modelos con offset
                ...
                return f"y = {get_param(0)}*exp(-{get_param(1)}*x) + {get_param(2)}*exp(-((x-{get_param(3)})/{get_param(4)})²) + {get_param(5)}*exp(-((x-{get_param(6)})/{get_param(7)})²) + {get_param(8)}*exp(-((x-{get_param(9)})/{get_param(10)})²) + {get_param(11)}"
            
            elif model_name == "Motor Eléctrico Graeme Paterson con Offset 2D":
                return f"y = {get_param(0)}*exp(-{get_param(1)}*t)*cos({get_param(2)}*t + {get_param(3)}) + {get_param(4)}*exp(-{get_param(5)}*t) + {get_param(6)}"
            
            elif model_name == "Klimpel Cinética Flotación A con Offset 2D":
                return f"y = {get_param(0)} * [1 - (1 - exp(-{get_param(1)}*x)) / ({get_param(1)}*x)] + {get_param(2)}"
            
            elif model_name == "Maxwell-Wiechert 1 con Offset 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}"
            
            elif model_name == "Maxwell-Wiechert 2 con Offset 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}*exp(-X/{get_param(3)}) + {get_param(4)}"
            
            elif model_name == "Maxwell-Wiechert 3 con Offset 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}*exp(-X/{get_param(3)}) + {get_param(4)}*exp(-X/{get_param(5)}) + {get_param(6)}"
            
            elif model_name == "Maxwell-Wiechert 4 con Offset 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}*exp(-X/{get_param(3)}) + {get_param(4)}*exp(-X/{get_param(5)}) + {get_param(6)}*exp(-X/{get_param(7)}) + {get_param(8)}"
            
            elif model_name == "Producción de Pozo Arps Modificado con Offset 2D":
                return f"y = ({get_param(0)}/((1.0-{get_param(1)})*{get_param(2)})) * (1.0-((1.0+{get_param(1)}*{get_param(2)}*x)^(1.0-1.0/{get_param(1)}))) + {get_param(3)}"
            
            elif model_name == "Ramberg-Osgood con Offset 2D":
                return f"y = (Stress/{get_param(0)}) + (Stress/{get_param(1)})^(1.0/{get_param(2)}) + {get_param(3)}"
            
            elif model_name == "Steinhart-Hart Extendido Recíproco con Offset 2D":
                return f"y = 1.0 / ({get_param(0)} + {get_param(1)}*ln(R) + {get_param(2)}*(ln(R))² + {get_param(3)}*(ln(R))³) + {get_param(4)}"
            
            elif model_name == "Steinhart-Hart Recíproco con Offset 2D":
                return f"y = 1.0 / ({get_param(0)} + {get_param(1)}*ln(R) + {get_param(2)}*(ln(R))³) + {get_param(3)}"
            
            elif model_name == "Sellmeier Óptico con Offset 2D":
                return f"y = 1 + ({get_param(0)}*x²)/(x²-{get_param(1)}) + ({get_param(2)}*x²)/(x²-{get_param(3)}) + ({get_param(4)}*x²)/(x²-{get_param(5)}) + {get_param(6)}"
            ...
            # Modelos con línea adicional
                ...
                return f"y = {get_param(0)} * [1 - (1 - exp(-{get_param(1)}*x)) / ({get_param(1)}*x)] + {get_param(2)}*x + {get_param(3)}"
            
            elif model_name == "Maxwell-Wiechert 1 más Línea 2D":
                return f"y = {get_param(0)}*exp(-X/{get_param(1)}) + {get_param(2)}*X + {get_param(3)}"
            ...
            print(f"Error generating equation for {model_name}: {str(e)}")
            # Devolver ecuación genérica como fallback
            equation = f"y = {self.get_model_function(model_name)[0].__name__}("
            equation += ", ".join([f"{name}={format_param(val)}" for name, val in zip(param_names, params)])
            equation += ")"
            return equation
        
    def _ensure_numeric_params(self, params):
        """Convierte parámetros a valores numéricos, manejando sets y otros tipos."""
        if isinstance(params, set):
            if len(params) == 1:
                return float(next(iter(params)))  # Tomar el único elemento
            else:
                # Intentar convertir todos los elementos y tomar el primero numérico
                for item in params:
                    try:
                        return float(item)
                    except (ValueError, TypeError):
                        continue
                return 0.0  # Valor por defecto
        elif isinstance(params, (list, np.ndarray, tuple)):
            return [self._ensure_numeric_params(p) for p in params]
        else:
            try:
                return float(params)
            except (ValueError, TypeError):
    ...
    def _ensure_list_params(self, params):
        """Convierte parámetros a lista si son set o numpy array"""
        if isinstance(params, (set, np.ndarray)):
            return list(params)
    ...
    def show_selected_stats(self):
        ...
        # Buscar el resultado CORRECTAMENTE - PRIORIZAR RESULTADOS MÁS RECIENTES
        result = None
        normalized_name = model_name.replace(" *", "")
        
        # Buscar de atrás hacia adelante para encontrar el resultado más reciente
        ...
        # SIEMPRE incluir bootstrap en las estadísticas si está disponible
        ...
        params_list = self._ensure_numeric_params(result["params"])
    ...
    R² ajustado: {1 - (1 - result["r2"]) * (len(result["x"]) - 1)/(len(result["x"]) - len(params_list) - 1):.6f}
    ...
    """
        ...
        # SIEMPRE añadir bootstrapping si existe - SIN CONDICIONES
    ...
    def ask_min_r2(self):
        ...
        if self.bootstrap_var.get():
            ...
            response = messagebox.askyesno(
                ...
                "¿Desea continuar procesando todos los modelos sin bootstrapping?"
            )
            
            if not response:
                # Si el usuario dice NO, desactivar bootstrap y salir
                self.bootstrap_var.set(False)
                self.toggle_bootstrap_settings()  # Esto actualizará la interfaz
                return
        
        # Continuar con la ventana normal para preguntar R² mínimo
    ...
    def show_selected_residuals(self):
        ...
        # Verificar que existen los datos necesarios para calcular residuos
        if "y_pred" not in result or "y" not in result:
            messagebox.showerror("Error", "No hay datos de predicción para calcular residuos")
            return
    ...
    def process_all_models(self, min_r2=0.0):
        ...
        # Guardar el estado original del bootstraping
        original_bootstrap_state = self.bootstrap_var.get()
        
        # Desactivar bootstrapping temporalmente si está activado
        if original_bootstrap_state:
            self.bootstrap_var.set(False)
            self.toggle_bootstrap_settings()  # Actualizar la interfaz
            ...
            for model in models:
                try:
                    ...
                    if last_result["r2"] >= min_r2:
                        ...
                        # Eliminar de la tabla pero mantener en resultados para consistencia
        ...
        # Restaurar el estado original del bootstraping
        if original_bootstrap_state:
    ...
    def show_selected_plot(self):
        ...
        # Siempre mostrar el gráfico principal
        self._show_main_fit_plot(result)
        
        # SIEMPRE mostrar distribuciones bootstrap si están disponibles
        if result.get("bootstrap_results"):
    ...
    def _show_main_fit_plot(self, result):
        ...
        def export_to_jpg():
            filepath = filedialog.asksaveasfilename(
                ...
                filetypes=[("Archivos JPG", "*.jpg"), ("Archivos JPEG", "*.jpeg"), ("Todos los archivos", "*.*")],
                title="Guardar gráfico como JPG"
            )
            ...
            try:
                ...
                fig.savefig(filepath, dpi=300, bbox_inches='tight', format='jpg')
        ...
        ttk.Button(button_frame, 
                   text="Exportar (.jpg)", 
                   command=export_to_jpg).pack(side=tk.LEFT, padx=5)
        ...
        # Configurar cierre correcto de la figura
        def on_close():
            plt.close(fig)
            plot_window.destroy()
    ...
    def _show_bootstrap_distributions(self, result):
        """Muestra gráficos de distribución de parámetros del bootstrap con opción de exportación"""
        bootstrap_params = result["bootstrap_results"]["bootstrap_params"]
        n_samples = len(next(iter(bootstrap_params.values())))
        ...
        # Frame principal
        main_frame = ttk.Frame(dist_window)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        notebook = ttk.Notebook(main_frame)
        notebook.pack(fill=tk.BOTH, expand=True)
        
        # Crear figura global para exportación
        global_fig = None
        ...
        # Función para exportar todos los parámetros en una sola imagen
        def export_all_bootstrap_jpg():
            filepath = filedialog.asksaveasfilename(
                defaultextension=".jpg",
                filetypes=[("Archivos JPG", "*.jpg"), ("Archivos JPEG", "*.jpeg"), ("Todos los archivos", "*.*")],
                title="Guardar gráficos bootstrap como JPG"
            )
            ...
            try:
                # Crear figura con todos los parámetros
                n_params = len(result["param_names"])
                n_cols = 2
                n_rows = (n_params + 1) // 2  # Redondear hacia arriba
                ...
                fig.suptitle(f"Distribuciones Bootstrap - {result['name']}", fontsize=16, y=0.98)
                ...
                for i, param_name in enumerate(result["param_names"]):
                    ...
                    # Histograma
                    ax.hist(param_values, bins=30, alpha=0.7, color='steelblue', density=True)
                    ...
                    ax.set_xlabel("Valor del parámetro", fontsize=10)
                    ...
                    stats_text = (f"Estimado: {param_estimate:.4f}\n"
                                 f"IC 95%: [{lower:.4f}, {upper:.4f}]\n"
                                 f"Error std: {np.std(param_values):.4f}")
                    ax.text(0.02, 0.98, stats_text, transform=ax.transAxes,
                ...
                messagebox.showinfo("Éxito", f"Gráficos bootstrap exportados correctamente a:\n{filepath}")
                
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo exportar los gráficos:\n{str(e)}")
        
        # Botón para exportar todos los parámetros
        ttk.Button(button_frame, 
                   text="Exportar Todos los Parámetros (.jpg)", 
                   command=export_all_bootstrap_jpg).pack(side=tk.LEFT, padx=5)
        
        # Botón de cerrar
        ttk.Button(button_frame, 
                   text="Cerrar", 
                   command=dist_window.destroy).pack(side=tk.RIGHT, padx=5)
        
        # Configurar cierre correcto de todas las figuras
        def on_close_all():
            # Cerrar todas las figuras de matplotlib asociadas a esta ventana
            plt.close('all')
            dist_window.destroy()
        
        dist_window.protocol("WM_DELETE_WINDOW", on_close_all)
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                ...
                for cat in sorted(categories):
                    ...
                    for result in cat_results:
                        # Asegurar que los parámetros sean completamente numéricos
                        numeric_params = []
                        for param in result['params']:
                            numeric_param = self._ensure_numeric_params(param)
                            numeric_params.append(numeric_param)
                        
                        
                        eval_text, _ = self.evaluate_r2(result["r2"])
                        
                        # Obtener la ecuación del modelo
                        equation = self.get_model_equation(result["name"], numeric_params, result["param_names"])
                        ...
                        try:
                            if 'pcov' in result and result['pcov'] is not None:
                                ...
                                perr = np.zeros_like(numeric_params)
                        except:
                            perr = np.zeros_like(numeric_params)
                        
                        for i, (name, param, err) in enumerate(zip(result["param_names"], numeric_params, perr)):
                            # Asegurar que param sea numérico
                            numeric_param = self._ensure_numeric_params(param)
                            numeric_err = self._ensure_numeric_params(err)
                            
                            ci_low = numeric_param - 1.96 * numeric_err
                            ci_high = numeric_param + 1.96 * numeric_err
                            
                            f.write(f"{name:<10} = {numeric_param:>12.6E} ± {numeric_err:.3E}\n")
                            f.write(f"{'IC 95% (normal)':<15} [{ci_low:.6E}, {ci_high:.6E}]\n")
                            
                            # Añadir información de bootstrap si existe
                            if (result.get("bootstrap_results") and 
                                name in result["bootstrap_results"]["conf_intervals"]):
                                ...
                                if name in result["bootstrap_results"]["bootstrap_params"]:
                                    bs_values = result["bootstrap_results"]["bootstrap_params"][name]
                                    # Filtrar valores no numéricos
                                    numeric_bs_values = [self._ensure_numeric_params(v) for v in bs_values]
                                    bs_std = np.std(numeric_bs_values) if numeric_bs_values else np.nan
    ...
    def export_plots_to_pdf(self):
        """Exporta los gráficos de los modelos seleccionados a un archivo PDF usando ReportLab"""
        # Obtener los modelos visibles en la tabla (que cumplen el criterio de R²)
        visible_models = []
        for item in self.tree.get_children():
            model_name = self.tree.item(item)['values'][0]
            normalized_name = model_name.replace(" *", "")
            result = next((r for r in self.results if r["name"] == normalized_name), None)
            if result:
                visible_models.append(result)
        
        if not visible_models:
            messagebox.showwarning("Advertencia", "No hay resultados visibles para exportar")
            return
        
        # Preguntar qué modelos exportar (usando solo los visibles)
        export_options = tk.Toplevel(self.root)
        export_options.title("Exportar Gráficos a PDF")
        export_options.geometry("500x400")
        export_options.resizable(False, False)
        export_options.grab_set()
        
        ttk.Label(export_options, text="Seleccione los modelos a exportar:", 
                  font=('Arial', 11, 'bold')).pack(pady=10)
        
        # Frame para la lista de modelos
        list_frame = ttk.Frame(export_options)
        ...
        # Llenar la lista con los modelos VISIBLES
        model_names = [f"{r['name']} (R²={r['r2']:.4f})" for r in visible_models]
        ...
        options_frame = ttk.Frame(export_options)
        options_frame.pack(fill=tk.X, padx=20, pady=10)
        
        include_residuals = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Incluir gráficos de residuos", 
                       variable=include_residuals).pack(anchor=tk.W)
        
        include_bootstrap = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Incluir distribuciones bootstrap (si disponibles)", 
                       variable=include_bootstrap).pack(anchor=tk.W)
    
        # Función para ejecutar la exportación (CORREGIDA: ahora es un método de la clase)
        def execute_export():
            ...
            try:
                # Usar los modelos VISIBLES seleccionados
                selected_results = [visible_models[i] for i in selected_indices]
                self._create_pdf_with_reportlab(filepath, selected_results, 
                                              include_residuals.get(), include_bootstrap.get())
        ...
        ttk.Button(button_frame, text="Exportar", 
                   style='Accent.TButton',
                   command=execute_export).pack(side=tk.RIGHT, padx=5)
        
        ttk.Button(button_frame, text="Cancelar", 
                   command=export_options.destroy).pack(side=tk.RIGHT, padx=5)
    ...
    def _create_pdf_with_reportlab(self, filepath, selected_results, include_residuals, include_bootstrap):
        """Crea el PDF usando ReportLab con tablas separadas para estadísticas y parámetros"""
        # Crear el documento
        ...
        # =========================================================================
        # PÁGINA 1: PORTADA CON INFORMACIÓN GENERAL
        # =========================================================================
        story.append(Paragraph("REPORTE DE GRÁFICOS - REGRESSION WIZARD", title_style))
        story.append(Spacer(1, 0.2*inch))
        
        # Información general
        info_text = f"""
        <b>Fecha de generación:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}<br/>
        <b>Total de modelos incluidos:</b> {len(selected_results)}<br/>
        <b>Total de puntos de datos:</b> {len(self.data['x'])}<br/>
        <b>Incluye análisis de residuos:</b> {'Sí' if include_residuals else 'No'}<br/>
        <b>Incluye análisis bootstrap:</b> {'Sí' if include_bootstrap else 'No'}<br/>
        <br/>
        """
        story.append(Paragraph(info_text, styles['Normal']))
        story.append(Spacer(1, 0.2*inch))
        
        # Lista de modelos con ecuaciones y R²
        story.append(Paragraph("<b>MODELOS INCLUIDOS EN EL REPORTE:</b>", styles['Heading2']))
        ...
        for i, result in enumerate(selected_results, 1):
            ...
            model_text = f"""
            <b>{i}. {result['name']}</b><br/>
            <b>Categoría:</b> {result['category']}<br/>
            <b>R²:</b> {result['r2']:.6f}<br/>
            <b>Ecuación:</b> {equation}<br/>
            <b>Parámetros estimados:</b> {', '.join([f'{n}={v:.4f}' for n,v in zip(result['param_names'], result['params'])])}<br/>
            """
            story.append(Paragraph(model_text, styles['Normal']))
            story.append(Spacer(1, 0.1*inch))
        
        # Añadir página en blanco para separar la portada del contenido
        story.append(PageBreak())
        
        # =========================================================================
        # PÁGINAS PARA CADA MODELO (1 modelo por 3 páginas)
        # =========================================================================
        for i, result in enumerate(selected_results, 1):
            # PÁGINA 1: GRÁFICO DEL MODELO Y PARÁMETROS
            story.append(Paragraph(f"Modelo {i}: {result['name']}", styles['Heading1']))
            story.append(Paragraph(f"Categoría: {result['category']}", styles['Heading2']))
            story.append(Spacer(1, 0.1*inch))
            
            # Gráfico principal
            main_plot_img = self._create_main_plot_image_compact(result, include_bootstrap)
            if main_plot_img:
                story.append(Paragraph("<b>Gráfico de Ajuste del Modelo:</b>", styles['Normal']))
                story.append(Spacer(1, 0.05*inch))
                story.append(main_plot_img)
                story.append(Spacer(1, 0.1*inch))
            
            # TABLA DE ESTADÍSTICAS (separada)
            stats_table = self._create_stats_table_separated(result)
            story.append(Paragraph("<b>Estadísticas del Modelo:</b>", styles['Normal']))
            story.append(Spacer(1, 0.05*inch))
            story.append(stats_table)
            story.append(Spacer(1, 0.1*inch))
            
            # TABLA DE PARÁMETROS (separada)
            params_table = self._create_parameters_table_separated(result)
            story.append(Paragraph("<b>Parámetros del Modelo:</b>", styles['Normal']))
            story.append(Spacer(1, 0.05*inch))
            ...
            # PÁGINA 2: BOOTSTRAP (si está disponible y seleccionado)
            if include_bootstrap and result.get("bootstrap_results"):
                story.append(Paragraph(f"Modelo {i}: {result['name']} - Análisis Bootstrap", styles['Heading1']))
                story.append(Spacer(1, 0.1*inch))
                
                bootstrap_img = self._create_bootstrap_image_compact(result)
                if bootstrap_img:
                    story.append(Paragraph("<b>Distribuciones Bootstrap de Parámetros:</b>", styles['Normal']))
                    story.append(Spacer(1, 0.05*inch))
                    story.append(bootstrap_img)
                    story.append(Spacer(1, 0.1*inch))
                
                # Estadísticas bootstrap
                bootstrap_stats = self._create_bootstrap_stats_table(result)
                if bootstrap_stats:
                    story.append(Paragraph("<b>Estadísticas Bootstrap:</b>", styles['Normal']))
                    story.append(Spacer(1, 0.05*inch))
                    story.append(bootstrap_stats)
                
                # Finalizar página 2
                story.append(PageBreak())
            else:
                # Si no hay bootstrap, dejar página en blanco con mensaje
                story.append(Paragraph(f"Modelo {i}: {result['name']}", styles['Heading1']))
                story.append(Spacer(1, 0.2*inch))
                story.append(Paragraph("<b>Análisis Bootstrap no disponible</b>", styles['Normal']))
                story.append(Paragraph("El análisis bootstrap no fue realizado o no está disponible para este modelo.", styles['Normal']))
                story.append(PageBreak())
            
            # PÁGINA 3: RESIDUOS (si está seleccionado)
            if include_residuals:
                story.append(Paragraph(f"Modelo {i}: {result['name']} - Análisis de Residuos", styles['Heading1']))
                story.append(Spacer(1, 0.1*inch))
                
                residuals_img = self._create_residuals_image_compact(result)
                if residuals_img:
                    story.append(Paragraph("<b>Gráficos de Análisis de Residuos:</b>", styles['Normal']))
                    story.append(Spacer(1, 0.05*inch))
                    story.append(residuals_img)
                    story.append(Spacer(1, 0.1*inch))
                
                # Estadísticas de residuos
                residuals_stats = self._create_residuals_stats_table(result)
                if residuals_stats:
                    story.append(Paragraph("<b>Estadísticas de Residuos:</b>", styles['Normal']))
                    story.append(Spacer(1, 0.05*inch))
                    story.append(residuals_stats)
            else:
                # Si no se incluyen residuos, dejar página en blanco con mensaje
                story.append(Paragraph(f"Modelo {i}: {result['name']}", styles['Heading1']))
                story.append(Spacer(1, 0.2*inch))
                story.append(Paragraph("<b>Análisis de Residuos no incluido</b>", styles['Normal']))
                story.append(Paragraph("El análisis de residuos no fue seleccionado para la exportación.", styles['Normal']))
            
            # Añadir salto de página después del último modelo, excepto para el último modelo
    ...
    def _create_stats_table_separated(self, result):
        """Crea una tabla separada solo con las estadísticas del modelo"""
        ...
        data = [
            ...
            ['RMSE', f"{rmse:.6f}"],
            ['MAE', f"{mae:.6f}"],
            ['N puntos', str(len(result["x"]))],
            ['N parámetros', str(len(result["params"]))]
        ]
    ...
    def _create_parameters_table_separated(self, result):
        """Crea una tabla separada solo con los parámetros del modelo"""
        # Datos de la tabla de parámetros
        data = [['Parámetro', 'Valor']]
        
        # Añadir cada parámetro
        for name, value in zip(result["param_names"], result["params"]):
            data.append([name, f"{value:.6f}"])
    ...
    def _create_main_plot_image(self, result, include_bootstrap):
        """Crea una imagen del gráfico principal usando matplotlib y la convierte para ReportLab"""
        try:
            # Crear figura más pequeña
            fig, ax = plt.subplots(figsize=(6, 4))  # Más pequeño que el original
            
            # Configuración del gráfico compacto
            ax.set_title(f"{result['name']}", fontsize=12, pad=10)
            ax.set_xlabel('X', fontsize=10)
            ax.set_ylabel('Y', fontsize=10)
            ax.grid(True, linestyle='--', alpha=0.7)
            
            # Datos originales (puntos más pequeños)
            ax.scatter(result["x"], result["y"], 
                      color='royalblue', label='Datos', 
                      s=30, alpha=0.7, edgecolors='w')
            
            # Curva ajustada (línea más delgada)
            x_vals = np.linspace(min(result["x"]), max(result["x"]), 300)  # Menos puntos
            y_vals = result["func"](x_vals, *result["params"])
            ax.plot(x_vals, y_vals, 'r-', linewidth=1.5, 
                   label='Ajuste', alpha=0.9)
            
            # Bandas de confianza bootstrap (si está disponible)
            if include_bootstrap and result.get("bootstrap_results"):
                bootstrap_params = result["bootstrap_results"]["bootstrap_params"]
                n_samples = len(next(iter(bootstrap_params.values())))
            ...
            ax.text(0.02, 0.98, f'R² = {result["r2"]:.4f}', transform=ax.transAxes,
                   fontsize=9, ha='left', va='top',
                   bbox=dict(boxstyle="round", fc="w", alpha=0.8))
            
            # Guardar la figura compacta
            buffer = io.BytesIO()
            plt.tight_layout()
            ...
            # Imagen más pequeña para el PDF compacto
            img = Image(buffer, width=5*inch, height=3*inch)
            return img
            
        except Exception as e:
            print(f"Error creando gráfico principal compacto: {e}")
            return None

    def _create_stats_table(self, result):
        """Crea una tabla con las estadísticas del modelo"""
        # Calcular métricas básicas
        residuals = result["y"] - result["y_pred"]
        rmse = np.sqrt(np.mean(residuals**2))
        mae = np.mean(np.abs(residuals))
        ...
        data = [
               ['Estadística', 'Valor', 'Parámetro', 'Valor'],
               [f'R²', f"{result['r2']:.4f}", '', ''],
               [f'RMSE', f"{rmse:.4f}", '', ''],
               [f'MAE', f"{mae:.4f}", '', ''],
               [f'N puntos', str(len(result["x"])), '', ''],
           ]
           
           # Añadir parámetros en dos columnas
        params = list(zip(result["param_names"], result["params"]))
        half = len(params) // 2 + len(params) % 2
           
        for i in range(half):
               row = ['', '']
               if i < len(params):
                   row.extend([params[i][0], f"{params[i][1]:.4f}"])
               else:
                   row.extend(['', ''])
               
               if i + half < len(params):
                   if i < len(data) - 5:  # Asegurar que tenemos espacio
                       data[5 + i].extend([params[i + half][0], f"{params[i + half][1]:.4f}"])
                   else:
                       # Añadir nueva fila si es necesario
                       new_row = ['', '', params[i + half][0], f"{params[i + half][1]:.4f}"]
                       data.append(new_row)
    ...
    def _create_residuals_image(self, result):
        """Crea una imagen con los gráficos de residuos"""
        try:
            ...
            # Subplot 1: Residuos vs X (más compacto)
            ax1.scatter(result["x"], residuals, alpha=0.6, color='blue', s=20)
            ax1.axhline(y=0, color='red', linestyle='--', linewidth=1.5)
            ax1.set_xlabel('X', fontsize=9)
            ax1.set_ylabel('Residuos', fontsize=9)
            ax1.set_title('Residuos vs X', fontsize=10)
            ax1.grid(True, alpha=0.3)
            ax1.tick_params(axis='both', which='major', labelsize=7)
            
            # Subplot 2: Residuos normalizados (más compacto)
            if len(residuals) > 1 and np.std(residuals) > 0:
                normalized_residuals = residuals / np.std(residuals)
            else:
                normalized_residuals = np.zeros_like(residuals)
            
            ax2.scatter(result["x"], normalized_residuals, alpha=0.6, color='green', s=20)
            ax2.axhline(y=0, color='red', linestyle='--', linewidth=1.5)
            ax2.axhline(y=2, color='orange', linestyle=':', linewidth=1)
            ax2.axhline(y=-2, color='orange', linestyle=':', linewidth=1)
            ax2.set_xlabel('X', fontsize=9)
            ax2.set_ylabel('Residuos Norm.', fontsize=9)
            ax2.set_title('Residuos Normalizados vs X', fontsize=10)
            ax2.grid(True, alpha=0.3)
            ax2.tick_params(axis='both', which='major', labelsize=7)
            
            plt.tight_layout()
            
            # Guardar figura compacta
            buffer = io.BytesIO()
            plt.savefig(buffer, format='PNG', dpi=150, bbox_inches='tight')
            buffer.seek(0)
            plt.close(fig)
            ...
            # Imagen más compacta
            img = Image(buffer, width=5*inch, height=4*inch)
            return img
            
        except Exception as e:
            print(f"Error creando gráficos de residuos compactos: {e}")
            return None
    
    def _create_bootstrap_image(self, result):
        """Crea una imagen con las distribuciones bootstrap"""
        try:
            if not result.get("bootstrap_results"):
                return None
            ...
            if n_params == 1:
                ...
                axes = [axes[0], axes[1]] if n_cols == 2 else [axes]
            else:
                axes = axes.flatten()
            
            fig.suptitle(f"Distribuciones Bootstrap", fontsize=12, y=0.95)
            
            for i, param_name in enumerate(result["param_names"]):
                if i >= len(axes) or param_name not in bootstrap_params:
                    continue
                ...
                # Histograma compacto
                ax.hist(param_values, bins=20, alpha=0.7, color='blue', density=True)
                
                # Líneas de referencia
                ax.axvline(param_estimate, color='red', linestyle='--', linewidth=1.5, label='Estimado')
                lower, upper = np.percentile(param_values, [2.5, 97.5])
                ax.axvline(lower, color='green', linestyle=':', linewidth=1, label='IC 95%')
            ...
            height = max(2.5, 2 * n_rows) * inch
            img = Image(buffer, width=6*inch, height=height)
            return img
            
        except Exception as e:
            print(f"Error creando gráficos bootstrap compactos: {e}")
            return None

    def _create_bootstrap_stats_table(self, result):
        """Crea tabla de estadísticas bootstrap"""
        if not result.get("bootstrap_results"):
            return None
        ...
        data = [['Parámetro', 'Estimado', 'IC 95% Inferior', 'IC 95% Superior', 'Error Std']]
        
        for param_name in result["param_names"]:
            if param_name in bootstrap_params:
                param_estimate = result["params"][result["param_names"].index(param_name)]
                lower, upper = conf_intervals.get(param_name, (np.nan, np.nan))
                std_error = np.std(bootstrap_params[param_name])
    ...
    def _create_main_plot_image_compact(self, result, include_bootstrap):
        """Versión compacta del gráfico principal para PDF"""
        return self._create_main_plot_image(result, include_bootstrap)
    
    def _create_parameters_table_compact(self, result):
        """Versión compacta de la tabla de parámetros para PDF"""
        # Calcular métricas básicas
        residuals = result["y"] - result["y_pred"]
        rmse = np.sqrt(np.mean(residuals**2))
        mae = np.mean(np.abs(residuals))
        
        # Datos de la tabla compacta
        data = [
            ['Estadística', 'Valor', 'Parámetro', 'Valor'],
            ['R²', f"{result['r2']:.4f}", '', ''],
            ['RMSE', f"{rmse:.4f}", '', ''],
        ...
        # Añadir parámetros en dos columnas
        params = list(zip(result["param_names"], result["params"]))
        half = (len(params) + 1) // 2  # Redondear hacia arriba
        
        for i in range(half):
            if i < len(params):
                param1_name, param1_val = params[i]
                param1_row = [param1_name, f"{param1_val:.4f}"]
            else:
                param1_row = ['', '']
                
            if i + half < len(params):
                param2_name, param2_val = params[i + half]
                param2_row = [param2_name, f"{param2_val:.4f}"]
            else:
                param2_row = ['', '']
            
            # Añadir fila con ambos parámetros
            if i < len(data) - 5:
                # Extender fila existente si hay espacio
                data[5 + i].extend(param1_row + param2_row)
            else:
                # Crear nueva fila
                new_row = ['', ''] + param1_row + param2_row
                data.append(new_row)
    ...
    def _create_bootstrap_image_compact(self, result):
        """Versión compacta del gráfico bootstrap para PDF"""
        return self._create_bootstrap_image(result)
    
    def _create_residuals_image_compact(self, result):
        """Versión compacta del gráfico de residuos para PDF"""
        return self._create_residuals_image(result)

    def _create_residuals_stats_table(self, result):
        """Crea tabla de estadísticas de residuos"""
        residuals = result["y"] - result["y_pred"]
        abs_errors = np.abs(residuals)
        
        if len(residuals) > 1 and np.std(residuals) > 0:
            normalized_residuals = residuals / np.std(residuals)
            outside_2sigma = np.sum(np.abs(normalized_residuals) > 2)
        else:
            outside_2sigma = 0
        
        data = [
            ['Estadística', 'Valor'],
            ['Media residuos', f"{np.mean(residuals):.4f}"],
            ['Desv. estándar', f"{np.std(residuals):.4f}"],
            ['Mínimo', f"{np.min(residuals):.4f}"],
            ['Máximo', f"{np.max(residuals):.4f}"],
            ['MAE', f"{np.mean(abs_errors):.4f}"],
            ['Fuera de ±2σ', f"{outside_2sigma} puntos"]
        ]
