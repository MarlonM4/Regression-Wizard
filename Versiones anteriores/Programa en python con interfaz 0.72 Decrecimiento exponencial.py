...
def gaussian_3params(x, a, b, x0):
    """Función gaussiana de 3 parámetros"""
    return a * np.exp(-0.5 * ((x - x0)/b)**2)

def gaussian_4params(x, a, b, x0, y0):
    """Función gaussiana de 4 parámetros con línea base y0"""
    return y0 + a * np.exp(-0.5 * ((x - x0)/b)**2)

def weibull_5param(x, a, b, c, x0, y0):
    """Función Weibull de 5 parámetros con línea base y0"""
    term = (c-1)/c
...
def weibull_4param(x, a, b, c, x0):
    """Función Weibull de 4 parámetros"""
    term = (c-1)/c
    threshold = x0 - b*(term)**(1/c)
    mask = x > threshold
    result = np.zeros_like(x)
...
def pseudo_voigt_5param(x, a, b, c, x0, y0):
    """Función Pseudo-Voigt de 5 parámetros con línea base y0"""
...
class ModelApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Regression Wizard - v0.71")
    ...
    def create_model_selection(self, parent):
        ...
        self.category_dropdown = ttk.Combobox(dropdown_frame, 
                                            ...
                                            values=["Sigmoidales", "Polinomiales", "Inversos polinomiales", "Modelos de campana", "Decaimiento Exponencial"],
                                            state="readonly",
                                            width=20)
    ...
    def update_model_list(self, event=None):
        ...
        elif category == "Decaimiento Exponencial":  # Nueva categoría
            models = [
    ...
    def get_initial_params(self, model_name, x, y):
        if model_name == "Exponencial simple 2p":
            return [np.max(y), 0.1]
