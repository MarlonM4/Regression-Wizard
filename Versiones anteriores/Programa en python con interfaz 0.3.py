...
# 1. DEFINICIÓN DE LOS 13 MODELOS
...
class ModelApp:
    def __init__(self, root):
        ...
        # Crear contenedor principal con scrollbar vertical
    ...
    def setup_styles(self):
        ...
        # Nuevo estilo para el botón de procesar
    ...
    def create_data_entry(self, parent):
        ...
        ttk.Label(control_frame, text="Año inicial:").pack(side="left", padx=5)
        ...
        ttk.Label(control_frame, text="Año final:").pack(side="left", padx=5)
        ...
        ttk.Button(control_frame, text="Generar Campos", 
                  command=self.generate_fields).pack(side="left", padx=10)
        
        # Área para campos de datos con scroll vertical y horizontal
        container_frame = ttk.Frame(entry_frame)
        container_frame.pack(fill="both", expand=True)
        
        self.data_canvas = tk.Canvas(container_frame, height=150)
        self.scrollbar_y = ttk.Scrollbar(container_frame, orient="vertical", command=self.data_canvas.yview)
        self.scrollbar_x = ttk.Scrollbar(container_frame, orient="horizontal", command=self.data_canvas.xview)
        self.data_frame = ttk.Frame(self.data_canvas)
        
        self.data_frame.bind(
            "<Configure>",
            lambda e: self.data_canvas.configure(
                scrollregion=self.data_canvas.bbox("all")
            )
        )
        
        self.data_canvas.create_window((0, 0), window=self.data_frame, anchor="nw")
        self.data_canvas.configure(yscrollcommand=self.scrollbar_y.set, xscrollcommand=self.scrollbar_x.set)
        
        # Organizar con grid dentro del container_frame
        ...
        # Botón para procesar
        ttk.Button(parent, text="PROCESAR MODELOS", style='Process.TButton',
    ...
    def process_models(self):
        ...
        # Definición de modelos con parámetros iniciales
        modelos = [
