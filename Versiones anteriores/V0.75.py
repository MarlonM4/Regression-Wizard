...
class ModelApp:
    ...
    def setup_window(self):
        self.root.title("Regression Wizard - v0.75")
    ...
    def process_selected_model(self, suppress_alerts=False):
        ...
        try:
            ...
            if not suppress_alerts:  # <- Solo mostrar alerta si suppress_alerts es False
            ...
            print(f"Modelo {model_name} ajustado. R² = {r2:.4f}")
    ...
    def process_all_models(self, min_r2=0.0):
        ...
        for model in models_to_process:
            ...
            try:
                # Ajustar el modelo (esto ya guarda resultados en self.results)
                self.process_selected_model(suppress_alerts=True)  # <- Añade este parámetro
                success_models += 1
    
                # Verificar si el último modelo añadido cumple R² ≥ min_r2
                last_result = self.results[-1]
                if last_result["r2"] >= min_r2:
                    filtered_models += 1
                else:
                    # Si no cumple, eliminarlo de la tabla (opcional: mantenerlo en self.results)
